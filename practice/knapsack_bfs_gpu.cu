...
const ssize_t THREADS_PER_BLOCK = 512;
...
__global__ void BranchGPU(const ssize_t* ind,
                const int* weightNode, const int* priceNode, const ssize_t* step, const ssize_t* last, const uint32_t* set,
                int* newWeightNode, int* newPriceNode, ssize_t* newStep, ssize_t* newLast, uint32_t* newSet,
                ssize_t stepCount, ssize_t blockCount, ssize_t n, int W, const int* weight, const int* price,
                ssize_t q) {
...
__global__ void BoundGPU(int* weightNode, int* priceNode, ssize_t* last, uint32_t* set,
                        int* lowerBound, int* upperBound, uint32_t* lowerBoundAdditionalSet,
                        ssize_t blockCount, ssize_t n, int W, const int* weight, const int* price,
                        ssize_t q) {
    ssize_t e = blockIdx.x * blockDim.x + threadIdx.x;
    if (e >= q) {
...
int main(int argc, char* argv[]) {
    ...
    weight[n] = price[n] = 0;
    int *cudaWeight = nullptr, *cudaPrice = nullptr;
    ...
    while (q > 0) {
        ...
        if (q > 1000000) {
            if (cudaWeight == nullptr) {
                cudaMalloc(&cudaWeight, (n + 1) * sizeof(*cudaWeight));
                cudaMalloc(&cudaPrice, (n + 1) * sizeof(*cudaPrice));
                cudaMemcpy(cudaWeight, weight, (n + 1) * sizeof(*cudaWeight), cudaMemcpyHostToDevice);
                cudaMemcpy(cudaPrice, price, (n + 1) * sizeof(*cudaPrice), cudaMemcpyHostToDevice);
            }
        
            int *cudaWeightNode, *cudaNewWeightNode;
            int *cudaPriceNode, *cudaNewPriceNode;
            ssize_t *cudaStep, *cudaNewStep;            
            ssize_t *cudaLast, *cudaNewLast;
            uint32_t *cudaSet, *cudaNewSet;
            ssize_t *cudaInd;

            cudaMalloc(&cudaWeightNode, q * sizeof(*cudaWeightNode));
            cudaMalloc(&cudaPriceNode, q * sizeof(*cudaPriceNode));
            cudaMalloc(&cudaStep, q * sizeof(*cudaStep));            
            cudaMalloc(&cudaLast, q * sizeof(*cudaLast));
            cudaMalloc(&cudaSet, q * blockCount * sizeof(*cudaSet));
            cudaMalloc(&cudaInd, (q + 1) * sizeof(*cudaInd));

            cudaMemcpy(cudaWeightNode, weightNode, q * sizeof(*cudaWeightNode),cudaMemcpyHostToDevice);
            cudaMemcpy(cudaPriceNode, priceNode, q * sizeof(*cudaPriceNode), cudaMemcpyHostToDevice);
            cudaMemcpy(cudaStep, step, q * sizeof(*cudaStep), cudaMemcpyHostToDevice);
            cudaMemcpy(cudaLast, last, q * sizeof(*cudaLast), cudaMemcpyHostToDevice);
            cudaMemcpy(cudaSet, set, q * blockCount * sizeof(*cudaSet), cudaMemcpyHostToDevice);
            cudaMemcpy(cudaInd, ind, (q + 1) * sizeof(*cudaInd), cudaMemcpyHostToDevice);

            cudaMalloc(&cudaNewWeightNode, ind[q] * sizeof(*cudaNewWeightNode));
            cudaMalloc(&cudaNewPriceNode, ind[q] * sizeof(*cudaNewPriceNode));
            cudaMalloc(&cudaNewStep, ind[q] * sizeof(*cudaNewStep));            
            cudaMalloc(&cudaNewLast, ind[q] * sizeof(*cudaNewLast));
            cudaMalloc(&cudaNewSet, ind[q] * blockCount * sizeof(*cudaNewSet));

            ssize_t qBlock = (q + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
            BranchGPU<<<qBlock, THREADS_PER_BLOCK>>>(cudaInd,
                cudaWeightNode, cudaPriceNode, cudaStep, cudaLast, cudaSet,
                cudaNewWeightNode, cudaNewPriceNode, cudaNewStep, cudaNewLast, cudaNewSet,
                stepCount, blockCount, n, W, cudaWeight, cudaPrice, q);
            ...
            cudaFree(cudaWeightNode);
            cudaFree(cudaPriceNode);
            cudaFree(cudaStep);            
            cudaFree(cudaLast);
            cudaFree(cudaSet);            
            cudaFree(cudaInd);
            ...
            int* cudaLowerBound;
            int* cudaUpperBound;
            uint32_t* cudaLowerBoundAdditionalSet;
            cudaMalloc(&cudaLowerBound, q * sizeof(*cudaLowerBound));
            cudaMalloc(&cudaUpperBound, q * sizeof(*cudaUpperBound));
            cudaMalloc(&cudaLowerBoundAdditionalSet, q * blockCount * sizeof(*cudaLowerBoundAdditionalSet));
            cudaMemcpy(cudaLowerBoundAdditionalSet, lowerBoundAdditionalSet,
                        q * blockCount * sizeof(*cudaLowerBoundAdditionalSet),
                        cudaMemcpyHostToDevice);

            qBlock = (q + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
            BoundGPU<<<qBlock, THREADS_PER_BLOCK>>>(cudaNewWeightNode, cudaNewPriceNode, cudaNewLast, cudaNewSet,
                cudaLowerBound, cudaUpperBound, cudaLowerBoundAdditionalSet, blockCount, n, W, cudaWeight, cudaPrice, q);
            ...
            lowerBound = (int*)malloc(q * sizeof(*lowerBound));
            upperBound = (int*)malloc(q * sizeof(*upperBound));
            
            cudaMemcpy(weightNode, cudaNewWeightNode, q * sizeof(*weightNode), cudaMemcpyDeviceToHost);
            cudaMemcpy(priceNode, cudaNewPriceNode, q * sizeof(*priceNode), cudaMemcpyDeviceToHost);
            cudaMemcpy(step, cudaNewStep, q * sizeof(*step), cudaMemcpyDeviceToHost);            
            cudaMemcpy(last, cudaNewLast, q * sizeof(*last), cudaMemcpyDeviceToHost);
            cudaMemcpy(set, cudaNewSet, q * blockCount * sizeof(*set), cudaMemcpyDeviceToHost);
            cudaMemcpy(lowerBound, cudaLowerBound, q * sizeof(*lowerBound), cudaMemcpyDeviceToHost);
            cudaMemcpy(upperBound, cudaUpperBound, q * sizeof(*upperBound), cudaMemcpyDeviceToHost);    
            cudaMemcpy(lowerBoundAdditionalSet, cudaLowerBoundAdditionalSet,
                        q * blockCount * sizeof(*lowerBoundAdditionalSet),
                        cudaMemcpyDeviceToHost);        

            cudaFree(cudaNewWeightNode);
            cudaFree(cudaNewPriceNode);
            cudaFree(cudaNewStep);
            cudaFree(cudaNewLast);
            cudaFree(cudaNewSet);
            cudaFree(cudaLowerBound);
            cudaFree(cudaUpperBound);
            cudaFree(cudaLowerBoundAdditionalSet);
